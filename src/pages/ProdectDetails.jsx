...
const ProdectDetails = () => {
  ...
  const [sizePrice, setSizePrice] = useState(products[0].sizeAndPrice[0]);
  ...
  return (
    <>
              ...
              <div className="mt-5">
                ...
                <div className="mt-3.5 flex justify-center lg:justify-start space-x-[19px]">
                  {getBrandColor.map(({ color }, index) => (
                    <div
                      key={`${color}-${index}`}
                      className={`brand-color border-[${color}] ${
                        product.color === color ? "border" : ""
                      }`}
                      role="button"
                      onClick={() => setProduct(products[index])}
                    >
                      <div className={`inner-circle bg-[${color}]`}></div>
                    </div>
                  ))}
                </div>
                {/* Wrist Size */}
      ...
      {/*modal*/}
      {open && <CheckoutModal setOpen={setOpen} />}
    </>
  );
};
